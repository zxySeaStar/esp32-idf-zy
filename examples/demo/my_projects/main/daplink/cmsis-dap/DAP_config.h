...
#define CPU_CLOCK              160000000 // SystemCoreClock        ///< Specifies the CPU Clock in Hz
...
#define DAP_PACKET_SIZE        64 //128 //512 //64              ///< USB: 64 = Full-Speed, 1024 = High-Speed.
...
#else
__STATIC_INLINE void pin_out_init(gpio_num_t gpio_num)
{
...
__STATIC_INLINE void pin_out_od_init(gpio_num_t gpio_num)
{
    gpio_config_t io_conf = {};
    //disable interrupt
    io_conf.intr_type = GPIO_INTR_DISABLE;
...
__STATIC_INLINE void pin_in_init(gpio_num_t gpio_num,uint8_t mode)
{
    if(mode==1)
    {
        gpio_config_t io_conf = {};
...
__STATIC_INLINE void PORT_SWD_SETUP(void)
{
    ...
    #else
    //pin_out_init(SWCLK_TCK_PIN_PORT);
    gpio_set_direction(SWCLK_TCK_PIN_PORT,GPIO_MODE_DEF_OUTPUT);
    gpio_set_level(SWCLK_TCK_PIN_PORT,GPIO_HIGH);

    //pin_out_init(SWDIO_OUT_PIN_PORT);
    gpio_set_direction(SWDIO_OUT_PIN_PORT,GPIO_MODE_DEF_OUTPUT);
    gpio_set_level(SWDIO_OUT_PIN_PORT,GPIO_HIGH);

    //pin_in_init(SWDIO_IN_PIN_PORT,1);

    pin_out_od_init(nRESET_PIN_PORT);
    gpio_set_level(nRESET_PIN_PORT,GPIO_HIGH);
...
__STATIC_INLINE void PORT_OFF(void)
{
    ...
    #else
    pin_in_init(SWCLK_TCK_PIN_PORT , 0);
    pin_in_init(SWDIO_OUT_PIN_PORT, 0);
    pin_in_init(SWDIO_IN_PIN_PORT, 0);
    #endif
}
...
__STATIC_FORCEINLINE uint32_t PIN_SWCLK_TCK_IN(void)
{
    ...
    #else
    return gpio_get_level(SWCLK_TCK_PIN_PORT);
    #endif
}
...
__STATIC_FORCEINLINE void PIN_SWCLK_TCK_CLR(void)
{
    ...
    #else
    gpio_set_level(SWCLK_TCK_PIN_PORT,GPIO_LOW);
     #endif
}
...
__STATIC_FORCEINLINE uint32_t PIN_SWDIO_TMS_IN(void)
{
    ...
    #else
    return gpio_get_level(SWDIO_IN_PIN_PORT);
    #endif
}
...
__STATIC_FORCEINLINE void PIN_SWDIO_TMS_CLR(void)
{
    ...
    #else
    gpio_set_level(SWDIO_OUT_PIN_PORT,GPIO_LOW);
    #endif
}
...
__STATIC_FORCEINLINE void PIN_SWDIO_OUT_DISABLE(void)
{
    ...
    SWDIO_OUT_PIN_PORT->BSRR = SWDIO_OUT_PIN;
    #else
    //pin_in_init(SWDIO_OUT_PIN_PORT,1);
    gpio_set_direction(SWDIO_OUT_PIN_PORT,GPIO_MODE_DEF_INPUT);
    gpio_set_level(SWDIO_OUT_PIN_PORT,GPIO_HIGH);
    #endif
}
...
__STATIC_FORCEINLINE uint32_t PIN_nRESET_IN(void)
{
    ...
    #else
    return gpio_get_level(nRESET_PIN_PORT);
    #endif
}
    ...
    if (bit & 1)
     ...
     if (bit & 1)
        gpio_set_level(nRESET_PIN_PORT,GPIO_HIGH);
         ...
         gpio_set_level(nRESET_PIN_PORT,GPIO_LOW);
    #endif
}
...
__STATIC_INLINE uint32_t TIMESTAMP_GET (void) {
  ...
  return 0;//esp_time_impl_get_time();
